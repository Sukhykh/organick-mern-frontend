        ...
        <div className={ styles.cartMenuItem }>
            <div className={ styles.cartMenuItem__product }>
                ...
                <div className={ styles.cartMenuItem__descr }>
                    <h3 className={ styles.cartMenuItem__title }>{product.title}</h3>
                    <div className={ styles.cartMenuItem__priceBar }>
                        {product.discount !== 0 && <span className={ styles.cartMenuItem__priceOld }>&#36;{ product.price.toFixed(2) }</span>}
                        <span className={ styles.cartMenuItem__priceCurent }>&#36;{ priceCurent }</span>
                    </div>
                </div>
            </div>
            <div className={ styles.cartMenuItem__cartLable }>
                <p className={ styles.cartMenuItem__cartTitle }>Quantity :</p>
                <input className={ styles.cartMenuItem__cartInput }
                    type="number" 
                    min={ 0 } 
                    name="quantity" 
                    id="quantity" 
                    value={quantity}
                    onChange={(e) => setRundomQuantity(product._id, Number(e.target.value))}
                    required
                />
                <a className={ styles.cartMenuItem__button } href="#" onClick={e => handleClick(e)}>X</a>
            </div>
        </div>
    )
}