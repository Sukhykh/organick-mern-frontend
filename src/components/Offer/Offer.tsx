...
export const Offer = () => {
    ...
    const curentCards = products.filter(el => el.tag === 'Vegetable').slice(0, 4)
            ...
            <div className={ styles.offer__container }>
                <div className={ styles.offer__wrapper }>
                    ...
                    <div className={ styles.offer__titleWrapper }>
                        <SectionTitle title='we offer organic for you' black={false} hero={false}/>
                    </div>
                    <div className={ styles.offer__cadrWrapper }>
                        {curentCards && curentCards.map(el => (
                            <ProductCard product={el} onClick={openModal} key={el._id}/>
                        ))}
                    </div>
                </div>
            </div>
