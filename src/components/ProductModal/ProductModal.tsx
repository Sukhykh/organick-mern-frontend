...
export const ProductModal: React.FC<ProductModalProps> = ({ onClick }) => {
    ...
    const [quantity, setQuantity] = useState<number>(getCartItemQuantity(dataId))
    const [isInfo, setIsInfo] =useState<boolean>(false)

    const priceCurent = data?.discount === 0 ? data?.price.toFixed(2) : data?.discount.toFixed(2);
    ...
    return (
        <div className={ styles.productModal }>
            <div className={ styles.productModal__container }>
                <a className={ styles.productModal__closeButton } href='#' onClick={ (e) => handleClick(e) }>X</a>
                <div className={ styles.productModal__wrapper }>
                    <div className={ styles.productModal__productWrapper }>
                        <div className={ styles.productModal__imgWrapper }>
                            <img className={ styles.productModal__img } 
                                src={ `${baseUrl}${data?.image}` }
                                alt={ data?.title } 
                                width={ 600 }
                                height={ 563 }
                            />
                            <div className={ styles.productModal__tag }>{ data?.tag}</div>
                            </div>
                            ...
                            <div className={ styles.productModal__price }>
                                ...
                                <span className={ styles.productModal__priceCurent }>&#36;{ priceCurent }</span>
                            </div>
                            <p className={ styles.productModal__description }>{ data?.description}</p>
                            <div className={ styles.productModal__cart }>
                                <div className={ styles.productModal__cartLable }>
                                    <p className={ styles.productModal__cartTitle }>Quantity :</p>
                                    <input className={ styles.productModal__cartInput }
                                        type="number" 
                                        min={ 0 } 
                                        name="quantity" 
                                        id="quantity" 
                                ...
                                <a className={ styles.productModal__cartButton } href="#" onClick={e => updateCart(e)}>
                                    <span className={ styles.productModal__buttonText } >Add to cart</span>
                                    <span className={ styles.productModal__buttonItem }>
                                        <svg className={ styles.productModal__buttonSvg }>
                                            <use className={ styles.productModal__buttonIcon } xlinkHref={ `${ sprite }#buttonArrow` } />
                                        </svg>
                                    </span>
                                </a>
                            </div>
                    ...
                    <div className={ styles.productModal__info }>
                        <div className={ styles.productModal__infoButtonBar }>
                            <a className={ !isInfo ? 
                                ...
                                href="#"
                                onClick={e => handleToggle(e)}>Product Description</a>
                            <a className={ isInfo ? 
                                `${styles.productModal__infoButton} ${styles.productModal__infoButton_activ}` : 
                                styles.productModal__infoButton } 
                                href="#"
                                onClick={e => handleToggle(e)}>Additional Info</a>
                        </div>
                        <p className={ styles.productModal__infoText }>{isInfo ? data?.additionalInfo : data?.productDescription }</p>
                    </div>
                </div>
            </div>
        </div>
