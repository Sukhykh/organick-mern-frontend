            ...
            <div className={ styles.testimonial__background }>
                <div className={ styles.testimonial__leftWrapper }>
                    <picture>
                        ...
                        <img className={ styles.testimonial__leftImg } 
                            src={ leftPng } 
                            alt="backgroung-left" 
                            width={ 484 }
                            height={ 1267 }
                ...
                <div className={ styles.testimonial__rightWrapper }>
                    <picture>
                        ...
                        <img className={ styles.testimonial__rightImg } 
                            src={ rightPng } 
                            alt="backgroung-left" 
                            width={ 432 }
                            height={ 1267 }
            ...
            <div className={ styles.testimonial__container }>
                <div className={ styles.testimonial__wrapper }>
                    ...
                    <div className={ styles.testimonial__titleWrapper }>
                        <SectionTitle title='what our customer saying?' black hero={false}/>
                    </div>
                    <div className={ styles.testimonial__content }>
                        {testimonialData && testimonialData.map(card => card.title === isActiv && <TestimonialCard data={ card } key={card.id}/>)}
                        <div className={ styles.testimonial__dotController }>
                            {testimonialData && testimonialData.map(card => (
                                <div className={ card.title === isActiv? `${styles.testimonial__dots} ${styles.testimonial__dots_activ}`: styles.testimonial__dots } onClick={() => setActivFrame(card.title) } key={card.title}></div>
                            ))}
                        </div>
                    </div>
                    <div className={ styles.testimonial__divider }></div>
                    <div className={ styles.testimonial__counterWrapper }>
                        {countersData && countersData.map(el => <TestimonialCounter data={el} key={el.id}/>)}
                    </div>
                </div>
            </div>
        </section>
